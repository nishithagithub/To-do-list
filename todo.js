...
// Function to initialize the data from localStorage
function initFromLocalStorage() {
    const todoList = retrieveData('todoList');
    if (todoList) {
        // Parse the JSON string and update the UI with the stored data
        const parsedList = JSON.parse(todoList);
        // Update your UI using parsedList
        // For example:
        $('#ulTodo').html(parsedList);
    }
}

// Function to save the current state to localStorage
function saveToLocalStorage() {
    const todoListHtml = $('#ulTodo').html();
    storeData('todoList', JSON.stringify(todoListHtml));
}

// Call initFromLocalStorage to load data on page load
initFromLocalStorage();

// Call saveToLocalStorage whenever you want to save the current state
// For example, after adding, updating, or deleting a todo item
saveToLocalStorage();
